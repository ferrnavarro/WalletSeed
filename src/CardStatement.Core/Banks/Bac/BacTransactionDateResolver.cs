...
public sealed class BacTransactionDateResolver
{
    private readonly StatementPeriod _period;

    public BacTransactionDateResolver(StatementPeriod period)
    {
        _period = period;
    }
    ...
    public DateOnly ResolveTransactionDate(string token)
    {
        ...
        if (parts.Length != 2 || !BacSpanishMonths.TryGet(parts[0], out var month))
