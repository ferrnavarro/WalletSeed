...
public sealed partial class BacTransactionRowParser
{
    ...
    private readonly BacTransactionDateResolver _dates;
    private readonly ColumnBands _bands;

    public BacTransactionRowParser(BacTransactionDateResolver dates, ColumnBands bands)
    {
        _dates = dates;
        _bands = bands;
    }

    public Transaction Parse(BacTableRow row, string cardLast4)
    {
        ...
        var amount = BacAmountParser.Parse(amountToken.Text);
    ...
    private static int FindAmountIndex(IReadOnlyList<PdfWord> words)
    {
        ...
        {
            if (BacAmountParser.TryParse(words[i].Text, out _))
                return i;
        }
        return -1;
    }
    ...
    private static string DebugText(BacTableRow row) =>
